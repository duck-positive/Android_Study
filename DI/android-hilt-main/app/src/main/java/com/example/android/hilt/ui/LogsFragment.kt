...
/**
* 위의 annotation을 적용하면 Android 클래스의 수명 주기를
* 따르는 종속 항목 컨테이너가 생성됨
*/
@AndroidEntryPoint
class LogsFragment : Fragment() {
    /**
    * 필드 삽입하려는 인스턴스 앞에 @Inject를 붙여줌
    * Hilt는 내부적으로 앞에서 생성한 컨테이너를 사용해서 이 필드들을 채워줌
    * 하지만 이렇게만 작성하면 Hilt는 해당 인스턴스를 어떻게 생성하는지 모름
    * 그렇기 때문에 해당 클래스의 생성자에도 @Inject를 붙여주는 것
    */
    @InMemoryLogger
